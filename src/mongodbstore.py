...
from utils import round_down_hour
import pymongo
...
class MongoDbStore():
    ...
    def __init__(self, db_connection: str, database: str, collection: str) -> None:
        ...
        self.mongo_collection.create_index("time", unique=True)
    
    
    async def get_by_time(self, time: datetime) -> TerrorZone:
        time = round_down_hour(time)
        tz = self.mongo_collection.find_one({"time": time})        
        if tz is None:
            return None        
        return TerrorZone(name=tz["name"], act=tz["act"], time=tz["time"])
    
    async def update(self, zone: TerrorZone) -> None:
        self.mongo_collection.update_one({"time": zone.time}, {"$set": zone.__dict__})
    
    async def set(self, zone: TerrorZone) -> None:
        if self.mongo_collection.find_one({"time": zone.time}) is None:
            return self.mongo_collection.insert_one(zone.__dict__)
        else:
            return self.mongo_collection.update_one({"time": zone.time}, {"$set": zone.__dict__})
